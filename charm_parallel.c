...
int main(void) {
  ...
  char *env_out_path = getenv("OUT_PATH");
  if (env_out_path == NULL) {
    printf("OUT_PATH is missing, result won't be saved or shown\n");
  }
  ...
  int remainder_min_support = min_support % size;
  ...
  while ((1 << (num_iteration - 1)) < size) {
    ...
    if (rank % (2 * distance) != 0) {
      ...
      if (source < size) {
        ...
        printf("rank %d local_min_support %d\n", rank, local_min_support);
